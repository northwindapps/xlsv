...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate,UITextFieldDelegate,UITextViewDelegate,MFMailComposeViewControllerDelegate,UICollectionViewDelegateFlowLayout,UIDocumentPickerDelegate,UIGestureRecognizerDelegate, BannerViewDelegate{
    ...
    // for CSV mode specifically, this fires on every keystroke-commit, and for a
    // 100k-row file that's a multi-MB JSON rebuild+write blocking the UI thread on
    // every edit. Serial (not concurrent) so saves from rapid consecutive edits
    // still land in the order they were issued -- a concurrent queue could let an
    // older edit's save finish after a newer one and silently clobber it back to
    // stale content.
    private let localJsonSaveQueue = DispatchQueue(label: "com.xlsv.viewcontroller.localJsonSave")
...
//    var cursor = String()
    ...
    // Programmatically-added button that opens the format panel (Fview). Sits just
...
//    var sheetIdx = 0
    ...
    // "#RRGGBB" for each name the format panel can produce -- what gets recorded
    // into pendingStyleChanges / handed to StyleTableEditor when styling an xlsx
    // cell (styles.xml only speaks explicit rgb). Kept in step with
    // namedCellColors above.
    ...
    func cellColorHex(_ name: String) -> String? {
        ...
        return ViewController.namedCellColorHex[name]
    }
    ...
    // Deferred-write tracking for in-app cell styling (text color / fill color /
    // font size -- see PendingStyleChangeSet.swift + StyleTableEditor.swift).
    // Kept separate from pendingXlsxChanges because these are resolved through
    // StyleTableEditor (one styles.xml rewrite for the whole batch) at flush time,
    // not spliced per-edit. Cleared alongside pendingXlsxChanges everywhere.
...
//            if selectedSheet >= 0{
    ...
    func flushPendingXlsxChangesIfNeeded() -> Bool {
        let appd: AppDelegate = UIApplication.shared.delegate as! AppDelegate
        guard pendingXlsxChanges.isDirty || appd.pendingCellSizeChanges || pendingStyleChanges.isDirty else { return true }
        ...
        let serviceInstance = Service(imp_sheetNumber: 0, imp_stringContents: [String](), imp_locations: [String](), imp_idx: [Int](), imp_fileName: "", imp_formula: [String]())
        let ok = serviceInstance.flushPendingEditsToXlsx(fp: local_xlsx_file_path, edits: pendingXlsxChanges.edits, columnWidths: columnWidths, rowHeights: rowHeights, sizeSheetIndex: appd.wsSheetIndex, styleChanges: pendingStyleChanges.edits)
        if ok {
...
//
    ...
    func updateUnsavedDataReminderVisibility() {
        let appd: AppDelegate = UIApplication.shared.delegate as! AppDelegate
        unsavedDataReminder.isHidden = !pendingXlsxChanges.isDirty && !appd.pendingCellSizeChanges && !pendingStyleChanges.isDirty
    }
    ...
    @objc func rowInsertOperation() {
        if isExcel {
            ...
            let serviceInstance = Service(imp_sheetNumber: 0, imp_stringContents: [String](), imp_locations: [String](), imp_idx: [Int](), imp_fileName: "",imp_formula:[String]())
            let rlt = serviceInstance.testRangeOperationsBox(fp: local_xlsx_file_path,content: content, locationInExcel:locationInExcel, styleIds: cellStyleId, formulaXmls: cellFormulaXml, insertedRowRange: (min: minRow, count: numberOfRowsToInsert) )
    ...
    @objc func rowDeleteOperation() {
        if isExcel {
            ...
            let serviceInstance = Service(imp_sheetNumber: 0, imp_stringContents: [String](), imp_locations: [String](), imp_idx: [Int](), imp_fileName: "",imp_formula:[String]())
            let rlt = serviceInstance.testRangeOperationsBox(fp: local_xlsx_file_path,content: content, locationInExcel:locationInExcel, styleIds: cellStyleId, formulaXmls: cellFormulaXml, deletedRowRange: (min: minRow, count: numberOfRowsToDelete) )
    ...
    @objc func columnInsertOperation(){
        if isExcel {
            ...
            let serviceInstance = Service(imp_sheetNumber: 0, imp_stringContents: [String](), imp_locations: [String](), imp_idx: [Int](), imp_fileName: "",imp_formula:[String]())
            let rlt = serviceInstance.testRangeOperationsBox(fp: local_xlsx_file_path,content: content, locationInExcel:locationInExcel, styleIds: cellStyleId, formulaXmls: cellFormulaXml, insertedColumnRange: (min: minCol, count: numberOfColsToInsert) )
    ...
    @objc func columnDeleteOperation() {
        if isExcel {
            ...
            let serviceInstance = Service(imp_sheetNumber: 0, imp_stringContents: [String](), imp_locations: [String](), imp_idx: [Int](), imp_fileName: "",imp_formula:[String]())
            let rlt = serviceInstance.testRangeOperationsBox(fp: local_xlsx_file_path,content: content, locationInExcel:locationInExcel, styleIds: cellStyleId, formulaXmls: cellFormulaXml, deletedColumnRange: (min: minCol, count: numberOfColsToDelete) )
    ...
    @objc func clearSelectedCellContent(){
        if isExcel {
            ...
            let serviceInstance = Service(imp_sheetNumber: 0, imp_stringContents: [String](), imp_locations: [String](), imp_idx: [Int](), imp_fileName: "",imp_formula:[String]())
            let rlt = serviceInstance.testRangeOperationsBox(fp: local_xlsx_file_path,content: content, locationInExcel:locationInExcel, styleIds: cellStyleId, formulaXmls: cellFormulaXml )
    ...
    @objc func copyPasteSelectedCellContent() {
        if isExcel && currentindex != nil {
            ...
            alert.addAction(UIAlertAction(title: "Yes", style: .default) { _ in
                ...
                let serviceInstance = Service(imp_sheetNumber: 0, imp_stringContents: [String](), imp_locations: [String](), imp_idx: [Int](), imp_fileName: "",imp_formula:[String]())
                let rlt = serviceInstance.testRangeOperationsBox(fp: self.local_xlsx_file_path,content: self.content, locationInExcel:self.locationInExcel, styleIds: self.cellStyleId, formulaXmls: self.cellFormulaXml )
    ...
    @objc func sliderValueChanged(_ sender:Any){
        ...
        // (see PendingStyleChangeSet / StyleTableEditor).
        if isExcel {
            let appd: AppDelegate = UIApplication.shared.delegate as! AppDelegate
...
//    }
    ...
    {
        // CSV sheets persist styling into their JSON sidecar; xlsx sheets record
        // into pendingStyleChanges and flush on Save/Export (see
        // flushPendingXlsxChangesIfNeeded / StyleTableEditor).
        if isCSV {
            saveAsLocalJson(filename: "csv_sheet1")
        }
        updateUnsavedDataReminderVisibility()
    ...
    func storeInput(IPd:String, elementd:String)
    {
        ...
        if elementd.replacingOccurrences(of: " ", with: "").count > 0{
            if let i = locationIndex(for: IPd) {
                ...
                location.append(IPd)
                // cellStyleId must stay in lockstep with content/location -- a
                // new cell that skips this leaves cellStyleId.count one short,
                // which makes resolveCellStyles() bail (silently) for EVERY
                // cell, not just this one (see the guard at the top of
                // resolveCellStyles()). A brand-new cell typed by the user
                // (as opposed to one loaded from the original xlsx) has no
                // explicit style, matching cellStyleId's own "" convention.
                cellStyleId.append("")
                // Same lockstep requirement as cellStyleId above, and the
                // same reasoning: a brand-new user-typed cell has no
                // preserved formula fragment to carry forward.
                cellFormulaXml.append("")
...
//        content = elsvFormulaExpression(src:content)
    ...
    // in the grid. For xlsx it grows every array kept index-aligned with
    // `location` (the same set the deferred-write reload path re-establishes),
    // so cellStyleId / cellFormulaXml / locationInExcel never desync -- see
    // Claude memory project_pendingxlsx_deferred_write_gap. Returns the slot index.
    @discardableResult
    ...
    func fonteditmode(){
        ...
        // xlsx: record the color change so it persists into styles.xml on save
        // (see PendingStyleChangeSet / StyleTableEditor). CSV keeps using the
        // JSON-sidecar path via formatbackaction -> saveAsLocalJson.
        if isExcel {
            let appd: AppDelegate = UIApplication.shared.delegate as! AppDelegate
...
//            }
            ...
            if sheet1Json.readJsonFile(title: "csv_sheet1"){
                ...
                // CustomCollectionViewLayout.prepare() floors its render grid on
                // appd.numberofRow/numberofColumn, which only xlsx import paths
                // set -- without this, CSV always rendered capped at the
    ...
    func initSheetData(){
        ...
        // entry in the first place -- so for CSV mode, content/location are
        // already guaranteed clean by construction, and this full O(n) re-scan
        // finds nothing to remove. Confirmed via instrumentation in
        // FileFillViewController: ~3.5s at 1.4M cells, running redundantly on
        // every single edit. xlsx mode doesn't have that guarantee, so it still
        // filters normally.
        if !isCSV {
            filterEmptyContent()
        }
        ...
        // unreachable in the grid. xlsx mode's existing behavior here is left
        // unchanged.
        if !isCSV {
            if (UserDefaults.standard.object(forKey: "NEWCsize") != nil) {
                COLUMNSIZE = UserDefaults.standard.object(forKey: "NEWCsize") as! Int
...
//        if selectedSheet >= 0{
    ...
    private func commitSpeechSegment(_ value: String, advance: Bool, completion: (() -> Void)? = nil){
        ...
        applyCellEditAndRefresh(idx: appd.wsSheetIndex) {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                if self.myCollectionView.collectionViewLayout is CustomCollectionViewLayout {
                    // see the matching comment in shouldChangeTextIn: reloadData already
                    // happened via patchJsonCacheAndRefresh, and animated:false avoids the
                    // per-frame prepare() cost of an animated re-center on large grids.
                    self.myCollectionView.selectItem(at: self.currentindex,
                                                     animated: false,
                                                     scrollPosition: [.centeredVertically, .centeredHorizontally])
                    self.collectionView(self.myCollectionView, didSelectItemAt: self.currentindex)
...
struct ExcelCell {
    ...
    let content: String
    // Empty string means "no style tracked for this cell" -- callers that
    // don't pass styleIds into testRangeOperationsBox leave every cell at
    // this default, same as before this field existed.
    let styleId: String
    // Empty string means "no preserved formula fragment for this cell" --
    // falls back to deriving formula/value from content, same as before
    // this field existed.
    let formulaXml: String
