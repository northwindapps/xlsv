...
class V2Service {
    ...
    func export(){
        FileManager.default.v2DeleteWorksheets()
              
              let semaphore = DispatchSemaphore(value: 1)
              DispatchQueue.global().async {
                  
                let adp = V2Adapter(imp_content: self.stringContents, imp_location: self.locations, imp_sheetIdx: self.sheetIdx, imp_sheetSize:self.sheetNumber,imp_formula:self.formulaContens)
                  ...
                  V2XlRels().export(sheetSize: self.sheetNumber)
              
                  V2ContentType().export(sheetSize: self.sheetNumber)
              
                  V2Workbook().export(sheetSize: self.sheetNumber)
              
                  V2Sharedstring(imp_sharedString: temp_string).export(sheetSize: self.sheetNumber)
                  
                  V2Sheet(imp_sheetContents: temp_ary).export(sheetSize: self.sheetNumber)
    ...
    func testExtractStyle(url:URL? = nil)->String?{
        if let url2 = url{
            ...
            let delegate = V2CustomXMLParserDelegate()
    ...
    func testStringUniqueAry(url:URL? = nil)->[String]?{
        if let url2 = url{
            ...
            if (xmlData != nil){
                let parser = XMLParser(data: xmlData!)
                // Set XMLParserDelegate
                let delegate = V2SharedStringsParserDelegate()
                parser.delegate = delegate
            ...
            if (xmlString != nil){
                ...
                if let idx = SSlist.firstIndex(of:word) {
                    ...
                    if let range = xmlString!.range(of: "</sst>") {
                        ...
                        let parser = XMLParser(data: xmlData!)
                        let delegate = V2XMLParserHelper()
                        parser.delegate = delegate
                        // Parse the XML data
                        if parser.parse() {
    ...
    func writeXlsx(path:URL,fileName:String){
        do{
            ...
            if FileManager.default.fileExists(atPath: zipFilePath.path) {
                ...
                FileManager.default.v2SecureCopyItem(at: URL(fileURLWithPath:zipFilePath.path), to:URL(fileURLWithPath: (path.appendingPathComponent(fileName).path)))
            }
        }
        catch
        {
...
extension FileManager {
    open func v2WriteXml(folder:String,filename:String,content:String) -> Bool{
    ...
    open func v2WriteXmlsandBox(folder:String,filename:String,content:String) -> Bool{
    ...
    open func v2GetFileURLsInFolder(folder: String) -> [URL]? {
    ...
    open func v2DeleteWorksheets() -> Bool{
    ...
    func v2UploadFileToICloud(url: URL) {
...
class V2XMLValidator: NSObject, XMLParserDelegate {
