...
let v2ReuseIdentifier = "customCell"
var V2SCREENSIZE_w = V2ScreenSize.SCREEN_WIDTH
var V2SCREENSIZE = V2ScreenSize.SCREEN_HEIGHT

// v2-only workspace-path handoff, previously piggybacked on AppDelegate.ws_path in the
// legacy app; kept off the shared AppDelegate since no v3 code reads it.
enum V2State {
    static var wsPath = ""
    static var wsIndex = 1
}

var otherclass = V2colorclass()


class V2ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate,UITextFieldDelegate,UITextViewDelegate,MFMailComposeViewControllerDelegate,UICollectionViewDelegateFlowLayout,UIDocumentPickerDelegate,UIGestureRecognizerDelegate,BannerViewDelegate{
    ...
    //var global = ns()
    var global2 = V2NilController()
    
    var boolean :Bool! //coulmnsize_check
    
    var numberview = V2numberkey()
    ...
    var customview2 :V2Customview2!
    var Fview :V2formatview!
    var datainputview :V2Datainputview!
    var Hintview:V2Hint!
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        //Render
        if collectionView === myCollectionView{
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: v2ReuseIdentifier, for: indexPath) as! V2CustomCollectionViewCell
            ...
            if location.contains(String(indexPath.item)+","+String(indexPath.section)){
                ...
                if indexPath.item == 0{
                    ...
                    cell.v2SetBorder(width: 0.8, color: UIColor.lightGray, sides: .bottom)
                    cell.label2?.textColor = UIColor.black
                    cell.label2?.textAlignment = .center
            ...
            if (styleId != nil && (appd.excelStyleIdx[styleId!] != -1) && appd.cellXfs.count != 0 && appd.numFmtIds.count != 0 && appd.numFmts.count != 0 && appd.excelStyleIdx.count != 0){
                ...
                if borderId < appd.border_lefts.count && appd.border_lefts[borderId] > 0{
                    cell.v2SetBorder(width: 0.8, color: UIColor.lightGray, sides: .left)
                    c+=1
                }
                
                if borderId < appd.border_rights.count && appd.border_rights[borderId] > 0{
                    cell.v2SetBorder(width: 0.8, color: UIColor.lightGray, sides: .right)
                    c+=1
                }
                
                if borderId < appd.border_tops.count && appd.border_tops[borderId] > 0{
                    cell.v2SetBorder(width: 0.8, color: UIColor.lightGray, sides: .top)
                    c+=1
                }
                ...
                if c == 0{
                    cell.v2SetBorder(width: 0.5, color: UIColor.lightGray, sides: .all)
                }
            }else{
                cell.v2SetBorder(width: 0.5, color: UIColor.lightGray, sides: .all)
            }
            ...
            //sheet cell
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as! V2FileCollectionViewCell
...
//                self.present(targetViewController, animated: true, completion: nil)
                ...
                if let customLayout = self.myCollectionView.collectionViewLayout as? V2CustomCollectionViewLayout {
                    //testing
...
//                    }
                    ...
                    print("V2CustomCollectionViewLayout is not set as the current layout")
                }
...
//
    ...
    func opendatainputview(){
        ...
        if datainputview != nil{
            ...
            case .phone:
                // It's an iPhone
                if Double(V2SCREENSIZE) != nil && Double(KEYBOARDLOCATION) != nil &&  V2SCREENSIZE > 0 &&  KEYBOARDLOCATION > 0 && Int(V2SCREENSIZE - KEYBOARDLOCATION - 60.0) > 0 {
                    // The result is an integer and greater than 0
                    datainputview = V2Datainputview(frame: CGRect(x:0,y:Int(V2SCREENSIZE - KEYBOARDLOCATION - 60.0), width: 320,height: 60))
                    
                } else {
                    datainputview = V2Datainputview(frame: CGRect(x:0,y:200, width: 320,height: 60))
                }
            ...
            case .pad:
                ...
                // It's an iPad
                datainputview = V2Datainputview(frame: CGRect(x:Int(60),y:Int(V2SCREENSIZE - KEYBOARDLOCATION - 160.0), width: 642,height: 160))
        ...
        datainputview.okbutton.addTarget(self, action: #selector(V2ViewController.terminate), for: UIControl.Event.touchUpInside)
        
        datainputview.returnbutton.addTarget(self, action: #selector(V2ViewController.restore), for: UIControl.Event.touchUpInside)
        
//        datainputview.copyButton.addTarget(self, action: #selector(V2ViewController.copyText), for: UIControl.Event.touchUpInside)
...
//        }
    ...
    @objc func showHint(){
        if Hintview != nil{
            if self.view.subviews.contains(Hintview){
                ...
                Hintview = V2Hint(frame: CGRect(x:Int(5),y:Int(10), width: 300,height: 330))
                Hintview.hintCloseButton.addTarget(self, action: #selector(V2ViewController.closeHview), for: UIControl.Event.touchUpInside)
    ...
    {
        //       postAction()
        let targetViewController = UIStoryboard(name: "V3", bundle: nil).instantiateViewController( withIdentifier: "creditView" )
    ...
    @objc func resetSheet(_ sender:UIButton){
        ...
        alert.addAction(UIAlertAction(title: yes, style: .default, handler: { action in
            ...
            let sheet1Json = V2ReadWriteJSON()
            ...
            let targetViewController = UIStoryboard(name: "V3", bundle: nil).instantiateViewController( withIdentifier: "LoadingViewController" )//Landscape
            targetViewController.modalPresentationStyle = .fullScreen
            self.present( targetViewController, animated: true, completion: nil)
...
//        bannerview.load(GADRequest())
        ...
        myCollectionView.v2ScrollToNextItem()
        ...
        // Only shown to users who actually have recovered legacy sheets waiting in
        // Documents/RecoveredFiles/ - everyone else sees no change at all.
    ...
    func addRecoveredFilesButton() {
        ...
        NSLayoutConstraint.activate([
            ...
            recoveredButton.trailingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.trailingAnchor, constant: -8)
        ])
    }

    @objc func showRecoveredFiles() {
    ...
    @IBAction func show2(_ sender: AnyObject) {
        ...
        switch tag_int {
        case 0:
            customview2 = V2Customview2(frame: CGRect(x:5,y:50, width: 285,height: 288))
            break
        case 1:
            customview2 = V2Customview2(frame: CGRect(x:5,y:50, width: 285,height: 288))
            break
        case 2:
            ...
            break
        case 3:
            customview2 = V2Customview2(frame: CGRect(x:5,y:10, width: 285,height: 288))
            break
        case 4:
            customview2 = V2Customview2(frame: CGRect(x:5,y:200, width: 285,height: 288))
            break
        case 5:
            customview2 = V2Customview2(frame: CGRect(x:5,y:190, width: 285,height: 288))
            break
        ...
        default:
            customview2 = V2Customview2(frame: CGRect(x:5,y:150, width: 285,height: 288))
            break
        ...
        customview2.export.addTarget(self, action: #selector(V2ViewController.movetosearchreplace(_:)), for: UIControl.Event.touchUpInside)
        
        customview2.calcAll.addTarget(self, action: #selector(fontediting), for: UIControl.Event.touchUpInside)
        
        customview2.back.addTarget(self, action: #selector(V2ViewController.back2(_:)), for: UIControl.Event.touchUpInside)
        
        customview2.localLoad.addTarget(self, action: #selector(V2ViewController.icloudview(_:)), for: UIControl.Event.touchUpInside)
        
        customview2.reset.addTarget(self, action: #selector(V2ViewController.resetSheet(_:)), for: UIControl.Event.touchUpInside)
        
        customview2.resetStyling.addTarget(self, action: #selector(V2ViewController.goSettings), for: UIControl.Event.touchUpInside)
        
        customview2.emailButton.addTarget(self, action: #selector(V2ViewController.excelEmail), for: UIControl.Event.touchUpInside)
        
        customview2.v135Data.addTarget(self, action: #selector(V2ViewController.showRecoveredFiles), for: UIControl.Event.touchUpInside)
        ...
        customview2.xlsxSheetExportOniCloudDrive.addTarget(self, action: #selector(V2ViewController.saveOniCloudAction), for: UIControl.Event.touchUpInside)
    ...
    @objc func createxlsxSheet(){
        ...
        let serviceInstance = V2Service(imp_sheetNumber: jsonFiles.count, imp_stringContents: export_content, imp_locations: export_location, imp_idx: export_sheet_idx, imp_fileName: "XLSV " + dateStr + ".xlsx",imp_formula:export_formula_location)
    ...
    @objc func fontediting() {
        ...
        Fview = V2formatview(frame: CGRect(x:10,y:30, width: 300,height: 150))
        ...
        Fview.formatBackButton.addTarget(self, action: #selector(V2ViewController.formatbackaction(_:)), for: UIControl.Event.touchUpInside)
        
        Fview.color1.addTarget(self, action: #selector(V2ViewController.c1(_:)), for: UIControl.Event.touchUpInside)
        Fview.color2.addTarget(self, action: #selector(V2ViewController.c2(_:)), for: UIControl.Event.touchUpInside)
        
        Fview.color5.addTarget(self, action: #selector(V2ViewController.c5(_:)), for: UIControl.Event.touchUpInside)
        Fview.color6.addTarget(self, action: #selector(V2ViewController.c6(_:)), for: UIControl.Event.touchUpInside)
        Fview.color7.addTarget(self, action: #selector(V2ViewController.c7(_:)), for: UIControl.Event.touchUpInside)
        Fview.color8.addTarget(self, action: #selector(V2ViewController.c8(_:)), for: UIControl.Event.touchUpInside)
        Fview.color9.addTarget(self, action: #selector(V2ViewController.c9(_:)), for: UIControl.Event.touchUpInside)
        Fview.color10.addTarget(self, action: #selector(V2ViewController.c10(_:)), for: UIControl.Event.touchUpInside)
        Fview.color11.addTarget(self, action: #selector(V2ViewController.c11(_:)), for: UIControl.Event.touchUpInside)
        Fview.color12.addTarget(self, action: #selector(V2ViewController.c12(_:)), for: UIControl.Event.touchUpInside)
        Fview.color13.addTarget(self, action: #selector(V2ViewController.c13(_:)), for: UIControl.Event.touchUpInside)
        Fview.color14.addTarget(self, action: #selector(V2ViewController.c14(_:)), for: UIControl.Event.touchUpInside)
        Fview.color15.addTarget(self, action: #selector(V2ViewController.c15(_:)), for: UIControl.Event.touchUpInside)
        Fview.sizeslider.addTarget(self, action: #selector(V2ViewController.sliderValueChanged(_:)), for: UIControl.Event.valueChanged)
    ...
    func numberviewopen() {
        ...
        numberview = V2numberkey(frame: CGRect(x:40,y:100, width: 210,height: 145))
        ...
        //
        numberview.back.addTarget(self, action: #selector(V2ViewController.backactionnum(_:)), for: UIControl.Event.touchUpInside)
        
        numberview.plusOne.addTarget(self, action: #selector(V2ViewController.plusAction(_:)), for: UIControl.Event.touchUpInside)
        
        
        numberview.minusOne.addTarget(self, action: #selector(V2ViewController.minusAction(_:)), for: UIControl.Event.touchUpInside)
    ...
    @objc func backactionnum(_ sender:UIButton)
    {
        ...
        let targetViewController = UIStoryboard(name: "V3", bundle: nil).instantiateViewController( withIdentifier: "LoadingFileController" ) as! LoadingFileController //Landscape
        if isExcel{
            targetViewController.idx = Int(appd.sheetNameIds[selectedSheet])
        }
        targetViewController.modalPresentationStyle = .fullScreen
...
//        }
    ...
    @objc func terminate(){
        if pastemode == false && getRefmode == false {
            ...
            for subview in self.view.subviews.filter({ $0 is V2Datainputview }){
...
//            }
    ...
    func show3() {
        ...
        switch tag_int {
        case 0:
            customview3 = V2Customview3(frame: CGRect(x:5,y:50, width: 250,height: 155))
            break
        case 1:
            customview3 = V2Customview3(frame: CGRect(x:5,y:50, width: 250,height: 155))
            break
        case 2:
            ...
            break
        case 3:
            customview3 = V2Customview3(frame: CGRect(x:5,y:10, width: 250,height: 155))
            break
        case 4:
            customview3 = V2Customview3(frame: CGRect(x:5,y:200, width: 250,height: 155))
            break
        case 5:
            customview3 = V2Customview3(frame: CGRect(x:5,y:190, width: 250,height: 155))
            break
        ...
        default:
            customview3 = V2Customview3(frame: CGRect(x:5,y:150, width: 235,height: 155))
            break
    ...
    @objc func excel_sum_each(fidx:Int,fc:[String],fl:[String],fle:[String],fr:[String],lc:[String],ll:[String],lle:[String],lr:[String])->String{
        if fc[fidx].hasPrefix("=SUM("){
            return V2ExcelHelper().excel_sum(src: fc[fidx], cursor:fl[fidx],fc: fc,fl: fl,fle: fle,fr: fr,lc: lc,ll:ll,lle: lle,lr: lr)
        }
    ...
    @objc func excel_average_each(fidx:Int,fc:[String],fl:[String],fle:[String],fr:[String],lc:[String],ll:[String],lle:[String],lr:[String])->String{
        if fc[fidx].hasPrefix("=AVERAGE("){
            return V2ExcelHelper().excel_average(src: fc[fidx], cursor:fl[fidx],fc: fc,fl: fl,fle: fle,fr: fr,lc: lc,ll:ll,lle: lle,lr: lr)
        }
        return "calculation error"
    }
    ...
    @objc func excel_min_each(fidx:Int,fc:[String],fl:[String],fle:[String],fr:[String],lc:[String],ll:[String],lle:[String],lr:[String])->String{
        if fc[fidx].hasPrefix("=MIN("){
            return V2ExcelHelper().excel_min(src: fc[fidx], cursor:fl[fidx],fc: fc,fl: fl,fle: fle,fr: fr,lc: lc,ll:ll,lle: lle,lr: lr)
        }
        return "calculation error"
    }
    ...
    @objc func excel_max_each(fidx:Int,fc:[String],fl:[String],fle:[String],fr:[String],lc:[String],ll:[String],lle:[String],lr:[String])->String{
        if fc[fidx].hasPrefix("=MAX("){
            return V2ExcelHelper().excel_max(src: fc[fidx], cursor:fl[fidx],fc: fc,fl: fl,fle: fle,fr: fr,lc: lc,ll:ll,lle: lle,lr: lr)
        }
        return "calculation error"
    }
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        if UIDevice.current.orientation.isLandscape {
            ...
            orientaion = "L"
            V2SCREENSIZE = size.height
            V2SCREENSIZE_w = size.width
        } else {
    ...
    func isExcelSheetData(sheetIdx:Int)->Bool{
        ...
        if isExcel{
            ...
            if localFileNames.count > 0 && sheet1Json.readJsonFile(title: "sheet" + String(V2State.wsIndex)){
                content = sheet1Json.content
                location = sheet1Json.location
                textsize = sheet1Json.fontsize
                bgcolor = sheet1Json.bgcolor
...
//        if selectedSheet >= 0{
    ...
    @objc func excelEmail() {
        ...
        if MFMailComposeViewController.canSendMail() {
            ...
            //print("V2ViewController" ,filePath)
...
extension FileManager {
    ...
    open func v2SecureCopyItem(at srcURL: URL, to dstURL: URL) -> Bool {
        do {
...
extension UIApplication {
    ...
    func v2MakeSnapshot() -> UIImage? { return v2GetKeyWindow()?.layer.v2MakeSnapshot() }
}


extension CALayer {
    func v2MakeSnapshot() -> UIImage? {
...
extension UICollectionView {
    ...
    func v2ScrollToNextItem() {
        let scrollOffset = CGFloat(floor(self.contentOffset.x + self.bounds.size.width))
        self.v2ScrollToFrame(scrollOffset: scrollOffset)
    }

    func v2ScrollToPreviousItem() {
        let scrollOffset = CGFloat(floor(self.contentOffset.x - self.bounds.size.width))
        self.v2ScrollToFrame(scrollOffset: scrollOffset)
    }

    func v2ScrollToFrame(scrollOffset : CGFloat) {
...
@IBDesignable class V2UIMarginLabel: UILabel {
...
extension Collection {
    func v2Distance(to index: Index) -> Int { distance(from: startIndex, to: index) }
}
extension UIView {
    func v2SetBorder(width: CGFloat, color: UIColor, sides: UIRectEdge) {
