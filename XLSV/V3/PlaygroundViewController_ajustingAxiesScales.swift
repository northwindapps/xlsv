...
//
//var fontcolorClass2 = colorclass()
//
...
//            
//        addAxes(to: scnView.scene!.rootNode, range:self.range)
//            
//        let resolution = 50
//        let width = range * 2
//        let offset = -range
...
//            for y in 0..<resolution {
//                for x in 0..<resolution {
//                    let xf = Float(x) / Float(resolution - 1) * width + offset
//                    let yf = Float(y) / Float(resolution - 1) * width + offset
//                    
//                    // 毎回元の式から新しく置換する
...
//                    let rawZf = Float(Double(res ?? "") ?? 0.0)
//                    let zf = rawZf * zScale // ユーザーが設定した倍率をかける
//                    
...
//    
//    // 軸を追加するヘルパー関数
//    private func addAxes(to rootNode: SCNNode, range: Float) {
//        // rangeが25なら、軸の長さは 25 * 2 = 50 に設定
//        let axisLength = CGFloat(range * 2)
//        let radius: CGFloat = 0.05 // 範囲が広い場合は少し太くすると見えやすい
//        let tickLength: CGFloat = 0.3
//        
//        func createText(_ text: String, color: UIColor, pos: SCNVector3, size: CGFloat = 0.5) {
...
//
//        // --- X軸 (赤) ---
//        let xNode = SCNNode(geometry: SCNCylinder(radius: radius, height: axisLength))
//        xNode.geometry?.firstMaterial?.diffuse.contents = UIColor.red
//        xNode.eulerAngles.z = Float.pi / 2
//        xNode.position = SCNVector3(0, 0, 0)
//        rootNode.addChildNode(xNode)
//        createText("X", color: .red, pos: SCNVector3(range + 1, 0, 0))
//
//        // --- Z軸 (緑: 高さ) ---
//        let zNode = SCNNode(geometry: SCNCylinder(radius: radius, height: axisLength))
//        zNode.geometry?.firstMaterial?.diffuse.contents = UIColor.green
//        zNode.position = SCNVector3(0, 0, 0)
//        rootNode.addChildNode(zNode)
//        createText("Z", color: .green, pos: SCNVector3(0, range + 1, 0))
//
//        // --- Y軸 (青: 奥行き) ---
//        let yNode = SCNNode(geometry: SCNCylinder(radius: radius, height: axisLength))
...
//        createText("Y", color: .blue, pos: SCNVector3(0, 0, range + 1))
//        
//        // 目盛りループ (rangeに合わせて動的に作成)
//        // range=25なら、ステップを5くらいにするとスッキリします
//        let step: Int = range > 10 ? 5 : 1
//
//        for i in stride(from: Int(-range), through: Int(range), by: step) {
//            if i == 0 { continue }
//            let val = Float(i)
//            
//            // --- X軸 ---
...
//            createText("\(i)", color: .red, pos: SCNVector3(val, -0.6, 0), size: 0.3)
//            
//            // --- Z軸 (高さ) ---
...
//            createText("\(i)", color: .green, pos: SCNVector3(-0.8, val, 0), size: 0.3)
//
//            // --- Y軸 (奥行き) ---
...
//            createText("\(i)", color: .blue, pos: SCNVector3(0, -0.6, val), size: 0.3)
//        }
//
//    }
//    
...
//                
//                // It's an iPad
//                datainputview = Datainputview(frame: CGRect(x:Int(60),y:Int(SCREENSIZE2 - KEYBOARDLOCATION - 160.0), width: 642,height: 160))
...
//        
//        datainputview.okbutton.addTarget(self, action: #selector(PlaygroundViewController.terminate), for: UIControl.Event.touchUpInside)
//        
//        datainputview.returnbutton.addTarget(self, action: #selector(PlaygroundViewController.restore), for: UIControl.Event.touchUpInside)
//        
////        datainputview.copyButton.addTarget(self, action: #selector(PlaygroundViewController.copyText), for: UIControl.Event.touchUpInside)
//        
...
//            message: "Enter function and scaling factors",
//            preferredStyle: .alert
//        )
//
//        // 1. 関数式の入力
//        alert.addTextField { textField in
//            textField.placeholder = "Function (e.g., x * y)"
//            textField.text = "x * y"
//        }
//
//        // 2. 表示範囲 (Range) の入力
//        alert.addTextField { textField in
//            textField.placeholder = "Range (e.g., 5)"
//            textField.text = "5"
...
//        // 3. 高さの倍率 (Z-Scale) の入力
//        alert.addTextField { textField in
//            textField.placeholder = "Z-Scale (e.g., 0.2)"
//            textField.text = "0.2"
//            textField.keyboardType = .decimalPad
//        }
//
...
//            let expression = fields?[0].text ?? "x * y"
//            let rangeStr = fields?[1].text ?? "5"
//            let scaleStr = fields?[2].text ?? "1"
//
//            // 数値に変換（失敗した時のためにデフォルト値を設定）
//            let range = Float(rangeStr) ?? 5.0
//            let zScale = Float(scaleStr) ?? 1.0
//
//            print("User entered: \(expression), Range: \(range), Scale: \(zScale)")
//            
//            // 引数を増やして setupGraphView を呼び出す
//            self.setupGraphView(expression: expression, range: range, zScale: zScale)
//        }
//
...
//    
// 
//    func setupGraphView(expression: String, range: Float, zScale: Float) {
...
//            self.bgcolor = []
//
//        let swiftUIView = SceneKitGraphView(
...
//    {
//        //It's now restoreing.
//        (content,location,COLUMNSIZE,ROWSIZE) = fontcolorClass2.outValues()
//        myCollectionView.reloadData()
//        
//    }
//    
...
//        
//        Fview.formatBackButton.addTarget(self, action: #selector(PlaygroundViewController.formatbackaction(_:)), for: UIControl.Event.touchUpInside)
//        
//        Fview.color1.addTarget(self, action: #selector(PlaygroundViewController.c1(_:)), for: UIControl.Event.touchUpInside)
//        Fview.color2.addTarget(self, action: #selector(PlaygroundViewController.c2(_:)), for: UIControl.Event.touchUpInside)
//        
//        Fview.color5.addTarget(self, action: #selector(PlaygroundViewController.c5(_:)), for: UIControl.Event.touchUpInside)
//        Fview.color6.addTarget(self, action: #selector(PlaygroundViewController.c6(_:)), for: UIControl.Event.touchUpInside)
//        Fview.color7.addTarget(self, action: #selector(PlaygroundViewController.c7(_:)), for: UIControl.Event.touchUpInside)
//        Fview.color8.addTarget(self, action: #selector(PlaygroundViewController.c8(_:)), for: UIControl.Event.touchUpInside)
//        Fview.color9.addTarget(self, action: #selector(PlaygroundViewController.c9(_:)), for: UIControl.Event.touchUpInside)
//        Fview.color10.addTarget(self, action: #selector(PlaygroundViewController.c10(_:)), for: UIControl.Event.touchUpInside)
//        Fview.color11.addTarget(self, action: #selector(PlaygroundViewController.c11(_:)), for: UIControl.Event.touchUpInside)
//        Fview.color12.addTarget(self, action: #selector(PlaygroundViewController.c12(_:)), for: UIControl.Event.touchUpInside)
//        Fview.color13.addTarget(self, action: #selector(PlaygroundViewController.c13(_:)), for: UIControl.Event.touchUpInside)
//        Fview.color14.addTarget(self, action: #selector(PlaygroundViewController.c14(_:)), for: UIControl.Event.touchUpInside)
//        Fview.color15.addTarget(self, action: #selector(PlaygroundViewController.c15(_:)), for: UIControl.Event.touchUpInside)
//        Fview.sizeslider.addTarget(self, action: #selector(PlaygroundViewController.sliderValueChanged(_:)), for: UIControl.Event.valueChanged)
//        
...
//        
//        //
//        numberview.back.addTarget(self, action: #selector(PlaygroundViewController.backactionnum(_:)), for: UIControl.Event.touchUpInside)
//        
//        numberview.plusOne.addTarget(self, action: #selector(PlaygroundViewController.plusAction(_:)), for: UIControl.Event.touchUpInside)
//        
//        
//        numberview.minusOne.addTarget(self, action: #selector(PlaygroundViewController.minusAction(_:)), for: UIControl.Event.touchUpInside)
//        
...
//        if indexSection == 0{
//            
//            (location,content) = fontcolorClass2.horribleMethod4Col(tempArray: location,tempArrayContent: content, colInt: indexItem)
//            
//            
//            plus = COLUMNSIZE+1
//            
...
//        }else if indexItem == 0{
//            
//            (location,content) = fontcolorClass2.horribleMethod4Row(tempArray: location,tempArrayContent: content, rowInt: indexSection)
//            
//            
//            plus = ROWSIZE+1
//            
...
//        if indexSection == 0{
//            
//            (location,content) = fontcolorClass2.horribleMethod4ColMinus(tempArray: location,tempArrayContent:content , colInt: indexItem)
//            
//            
//            minus = COLUMNSIZE-1
...
//        }else if indexItem == 0{
//            
//            (location,content) = fontcolorClass2.horribleMethod4RowMinus(tempArray: location,tempArrayContent: content, rowInt: indexSection)
//            
//            
//            minus = ROWSIZE-1
//            
...
//            
//            orientaion = "L"
//            SCREENSIZE2 = size.height
//            SCREENSIZE_w2 = size.width
//        } else {
//            
//            orientaion = "P"
//            SCREENSIZE2 = size.height
...
//            
//            
//            //print("PlaygroundViewController" ,filePath)
