        ...
        do {
            ...
            if FileManager.default.fileExists(atPath: fp) {
                ...
                if rIdValue != "" && snippet != ""{
                    ...
                    var sheetFiles = try FileManager.default.contentsOfDirectory(at: sheetDirectoryURL, includingPropertiesForKeys: nil)
                    let sheetXMLFiles = sheetFiles.filter { $0.pathExtension == "xml" }
                    let newFirstSheetStr = numberOnlyString(text: sheetXMLFiles.first?.lastPathComponent ?? "")
                    appd.wsSheetIndex = Int(newFirstSheetStr ?? "0") ?? 0
                    appd.wsIndex = Int(newFirstSheetStr ?? "0") ?? 0
                    appd.wsSheetIndex += 1
                    appd.wsIndex += 1
                    //xl/_res/workbook
                    let _relsDirectoryURL = subdirectoryURL.appendingPathComponent("xl").appendingPathComponent("_rels").appendingPathComponent("workbook.xml.rels")
...
//                    if let range_xlres2 = xmlString2?.range(of: pattern_xlres, options: .regularExpression) {
//                        let snippet_xlres2 = String(xmlString2![range_xlres2])
//                        print("snippet_xlres2",snippet_xlres2)
//                        xmlString2 = xmlString2?.replacingOccurrences(of: snippet_xlres2, with: "")
//                        try? xmlString2?.write(to: reRelsDirectoryURL, atomically: true, encoding: .utf8)
//                    } else {
//                        print("Relationship not found")
//                    }
                }
